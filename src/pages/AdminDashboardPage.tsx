...
const AdminDashboardPage: React.FC = () => {
  const { user } = useAuth();
  const { admins, addAdmin, removeAdmin, changePassword, ownerVerified } = useAdminAuth();
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-lg border p-5">
        ...
        {!ownerVerified && (
          <div className="mb-3 p-2 rounded bg-red-50 border text-red-800">Owner verification required on the Admin Sign-In page to manage admins.</div>
        )}

        <form onSubmit={handleAddAdmin} className="grid md:grid-cols-3 gap-3 mb-4">
          <input className="border rounded p-2" placeholder="Username" value={newAdmin.username} onChange={e => setNewAdmin(s => ({ ...s, username: e.target.value }))} disabled={!ownerVerified} />
          <input className="border rounded p-2" type="password" placeholder="Password" value={newAdmin.password} onChange={e => setNewAdmin(s => ({ ...s, password: e.target.value }))} disabled={!ownerVerified} />
          <input className="border rounded p-2" type="password" placeholder="Confirm Password" value={newAdmin.confirm} onChange={e => setNewAdmin(s => ({ ...s, confirm: e.target.value }))} disabled={!ownerVerified} />
          <button className="md:col-span-3 bg-blue-600 hover:bg-blue-700 text-white rounded p-2" disabled={!ownerVerified}>Add Admin</button>
        </form>
          ...
          {admins.length === 0 ? (
            ...
            <div key={a.username} className="flex items-center justify-between p-3">
              ...
              <div className="flex items-center gap-2">
                <button className="px-2 py-1 text-sm border rounded" onClick={() => handleChangePassword(a.username)} disabled={!ownerVerified}>
                  <KeyRound className="h-4 w-4" />
                </button>
                <button className="px-2 py-1 text-sm border rounded text-red-600" onClick={() => removeAdmin(a.username)} disabled={!ownerVerified}>
